...
bool gxHWShader::loadShader(const char* vShaderFile, const char* fShaderFile)
{
	...
	vertShaderPathname = getResourcePath(vShaderFile);
	...
	fragShaderPathname = getResourcePath(fShaderFile);
...
bool gxHWShader::compileShader(GLuint* shader, GLenum type, const char* file)
{
	...
	int fileSz=0;
	FILE* fp=fopen(objC_util::getPath(file), "r");
	fseek(fp, 0, SEEK_END);
	fileSz=ftell(fp);
	fclose(fp);
	...
	fp=fopen(objC_util::getPath(file), "r");
...
{
	stLinkNode<stAttribLocation*>* attribRefVarnode=m_vAttribRefVarList.getHead();
	while(attribRefVarnode)
	{
		stAttribLocation* attribRefVar=attribRefVarnode->getData();
		if(strcmp(attribRefVar->attrib_ref_name, avar)==0)
		{
			return attribRefVar->attrib_ref;
		}
		attribRefVarnode=attribRefVarnode->getNext();
	}
	...
	m_vAttribRefVarList.insertTail(newAttribRefVar);
...
void gxHWShader::clearAttribRefVarList()
{
	...
	m_vAttribRefVarList.clearAll();
}

int gxHWShader::getUniformLoc(const char* uvar)
{
	stLinkNode<stUniformLocation*>* uniformRefVarnode=m_vUniformRefVarList.getHead();
	while(uniformRefVarnode)
	{
		stUniformLocation* uniformRefVar=uniformRefVarnode->getData();
		if(strcmp(uniformRefVar->uniform_ref_name, uvar)==0)
		{
			return uniformRefVar->uniform_ref;
		}
		uniformRefVarnode=uniformRefVarnode->getNext();
	}
	...
	m_vUniformRefVarList.insertTail(newUnifromRefVar);
...
void gxHWShader::clearUniformRefVarList()
{
	...
	{
		stUniformLocation* uniformRefVar=uniformRefVarnode->getData();
		GX_DELETE(uniformRefVar);
		uniformRefVarnode=uniformRefVarnode->getNext();
	}
	m_vUniformRefVarList.clearAll();
}
